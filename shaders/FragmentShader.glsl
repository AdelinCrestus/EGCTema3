...
float f(vec3 L,vec3 Light_Pos, int ok)
{
...
float spotlight(vec3 Light_Dir,vec3 Light_Pos)
{
    float cut_off = 0.5f;
...
void main()
{
    ...
    float culoareObiect = f(vec3(0, -1, 0), world_position + vec3(0,2,0), 1);
    float culoareObiectpct = 0.0;
    vec3 objcolor = vec3(0);
    for(i = 1 ; i < N; i++)
    {
        culoareObiectpct = f(vec3(0, -1, 0), positions[i] + vec3(0,1,0), 0);
        objcolor += colors[i]*culoareObiectpct;
    }
    float culoareObiectSpot = 0.0f;
    for(int i = 0; i < M; i++)
    {
        culoareObiectSpot += spotlight(vec3(0, -1, 0),pos_lighting[i] + spotPos );
        culoareObiectSpot += spotlight(vec3(0, -1, 0), pos_lighting[i] + spotPos + vec3(-2*spotPos.x, 0, 0));
    }
